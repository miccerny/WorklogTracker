...
public class JWTService {
   ...
   public JWTService(@Value("${security.jwt.secret}") String secret,
       ...
       if(expirationMs <= 0){
           throw new IllegalArgumentException("security.jwt.expiration must be greater than 0");
       }
       this.accessTokenTtlMs = expirationMs;

       try{
           this.signingKey = Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8));
       }catch (WeakKeyException ex){
           throw  new IllegalStateException(
                   "security.jwt.secret is too short for HS256; use at least 32 ASCII characters", ex
           );
       }
   }
